...
function log_info ()
{
     ...
     echo "${DATE_N} ${USER_N} execute $0 [INFO] $@" >>/var/log/openstack-kilo/presystem_compute_node.log
...
function log_error ()
{
     ...
     echo -e "\033[41;37m ${DATE_N} ${USER_N} execute $0 [ERROR] $@ \033[0m"  >>/var/log/openstack-kilo/presystem_compute_node.log
...
function install-cluster-software()
{
	if [ ! -f /etc/openstack-kilo_tag/make_yumrepo.tag ]
	 ...
	 yum install pacemaker pacemaker-remote pcs corosync fence* resource* bind-utils tcpdump sos nfs-utils -y
...
function install_nfs (){
    ...
    if [ $? -eq 0 ]
        then
        echo "you do not need install nfs "
    else
        echo "your minial OS has not NFS installed,so we will install NFS rpm"
    fi
    ls -l /etc/yum.repos.d/centos7-iso-tmp.repo
    if [ $? -eq 0 ]
        then
        yum clean all && yum makecache
        yum install nfs-utils -y  
    else
        echo "nfs installation failed!"
        exit
    fi
...
function set_hostname() {
    ...
    echo "$INT_IP $HOST_NAME" >> /etc/hosts
}
...
if [ `hostname` != "$HOST_NAME" ]
    ...
    log_info "hostname already be setted"
fi
...
if [ $? -ne 0 ]
    ...
    log_info "nfs have already installed" 
fi
...
#finish
echo `date "+%Y-%m-%d %H:%M:%S"` >/etc/openstack-kilo_tag/presystem_compute_node.tag
