...
do
	echo "beging install mongodb-server on ${hacluster_node[i]}"
	ssh root@${hacluster_node[i]} "/usr/bin/bash vm_install_mongodb_and_setting_env.sh"
done

echo "create mongodb-server resource on pacemaker"
ssh root@$master_vm_name "/usr/bin/bash vm_create_monogdb_resource_on_pacemaker.sh"
