...
do
	echo "beging install neutron on ${hacluster_node[i]}"
	ssh root@${hacluster_node[i]} "/usr/bin/bash vm_install_neutron_and_config.sh"
done

echo "create neutron resource on pacemaker"
ssh root@$master_vm_name "/usr/bin/bash vm_create_neutron_resource_on_pacemaker.sh"
sleep 10
