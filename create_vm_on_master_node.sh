...
# 1. configure master controller node as  apache  http rpm download server when install vm
# 2. install&configure host fence(fence_virtd) so that guest can be fenced by fence_xvm with multicast cummuniciation
...
listeners {
        multicast {
                ...
                # 指定接口会使得guest端的fence_xvm -o list 一直timeout,即guest端发出的多播请求包得不到响应
...
if [ -f /etc/openstack-kilo_tag/create_base_vm.tag ] || [ -f ${vm_dir}/$vm_base ]
	then
	log_info "base image are already created "
else
	if [ -d ${vm_dir} ]
		then
		rm -rf ${vm_dir}
...
if [ -f /etc/openstack-kilo_tag/define_vm_xml.tag ]
	then
	log_info "the XML file already defined"
else
	if [ -d /localvms ]
		then
		rm -rf /localvms
