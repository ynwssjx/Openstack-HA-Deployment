...
do
	echo "beging install nova on ${hacluster_node[i]}"
	ssh root@${hacluster_node[i]} "/usr/bin/bash vm_install_nova_and_config.sh"
done

echo "create nova resource on pacemaker"
ssh root@$master_vm_name "/usr/bin/bash vm_create_nova_resource_on_pacemaker.sh"
sleep 10
