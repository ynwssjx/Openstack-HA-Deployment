...
do
	echo "beging install horizon on ${hacluster_node[i]}"
	ssh root@${hacluster_node[i]} "/usr/bin/bash vm_install_horizon_and_config.sh"
done

echo "create horizon resource on pacemaker"
ssh root@$master_vm_name "/usr/bin/bash vm_create_horizon_resource_on_pacemaker.sh"
sleep 10
