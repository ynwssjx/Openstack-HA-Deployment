...
do
	echo "beging install keystone on ${hacluster_node[i]}"
	ssh root@${hacluster_node[i]} "/usr/bin/bash vm_install_keystone_and_config.sh"
done

echo "create keystone resource on pacemaker"
ssh root@$master_vm_name "/usr/bin/bash vm_create_keystone_resource_and_keystone_init.sh"
