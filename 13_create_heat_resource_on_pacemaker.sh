...
do
	echo "beging install heat on ${hacluster_node[i]}"
	ssh root@${hacluster_node[i]} "/usr/bin/bash vm_install_heat_and_config.sh"
done

echo "create heat resource on pacemaker"
ssh root@$master_vm_name "/usr/bin/bash vm_create_heat_resource_on_pacemaker.sh"
sleep 10
