...
do
	echo "beging install cinder on ${hacluster_node[i]}"
	ssh root@${hacluster_node[i]} "/usr/bin/bash vm_install_cinder_and_config.sh"
done

echo "create cinder resource on pacemaker"
ssh root@$master_vm_name "/usr/bin/bash vm_create_cinder_resource_on_pacemaker.sh"
sleep 10
